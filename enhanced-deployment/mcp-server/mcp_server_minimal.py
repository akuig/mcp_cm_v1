#!/usr/bin/env python3
"""
MINIMAL TEST MCP Server - Exactly like original working version but with protocol fix
"""
...
class TelecomMCPServer:
    ...
    def get_tools(self) -> List[Dict]:
        """Return minimal test tools - use same format as working version"""
        tools = [
            {
...
async def mcp_stream_handler(request):
    """Minimal MCP stream handler - exactly match original working version"""
    client_info = "unknown"
    try:
        ...
        if method == "initialize":
            ...
            # Accept Claude's protocol version - this is the key fix
            client_version = body.get("params", {}).get("protocolVersion", "2024-11-05")
            
            # Match Claude's protocol version exactly
            ...
            logger.info("📋 🎉 CLAUDE REQUESTED TOOLS LIST! 🎉")
...
async def startup():
    ...
    logger.info(f"🚀 Starting MINIMAL TEST Telepath MCP Server on {HOST}:{PORT}")
