...
verify_data() {
    ...
    if command -v jq &> /dev/null; then
        ...
        log_warning "jq not installed, skipping detailed data verification"
        # Basic health check
        if curl -f http://localhost:8080/tmf633/serviceSpecification &>/dev/null; then
            log_success "Service specifications endpoint responding"
        fi
    fi
...
show_available_services() {
    ...
    if command -v jq &> /dev/null; then
        ...
        echo "Install jq for detailed service listing: brew install jq"
    fi
}
...
show_endpoints() {
    ...
    echo "Web Testing UI: http://localhost:8082"
